...
#${subjdir}/nodif_brain_mask.nii.gz ${subjdir}.bedpostX
...
# Split the dataset in parts 这里把原本的data改成了data.nii.gz
echo Pre-processing stage
if [ ${gflag} -eq 1 ]; then
	pre_command="$FSLDIR/bin/split_parts_gpu ${subjdir}/data.nii ${subjdir}/nodif_brain_mask ${subjdir}.bedpostX/bvals ${subjdir}.bedpostX/bvecs ${subjdir}/grad_dev 1 $njobs ${subjdir}.bedpostX"
else
	pre_command="$FSLDIR/bin/split_parts_gpu ${subjdir}/data.nii ${subjdir}/nodif_brain_mask ${subjdir}.bedpostX/bvals ${subjdir}.bedpostX/bvecs NULL 0 $njobs ${subjdir}.bedpostX"
fi

echo $pre_command > ${subjdir}.bedpostX/temp
chmod 755 ${subjdir}.bedpostX/temp

sh ${subjdir}.bedpostX/temp
...
chmod 755 ${subjdir}.bedpostX/commands.txt 
echo "command=\`cat "$taskfile" | head -\$PBS_ARRAYID | tail -1\` ; exec \$command" > ${subjdir}.bedpostX/temp
sh ${subjdir}.bedpostX/commands.txt

#while read line; do { $line ; } & done < ${subjdir}.bedpostX/commands.txt
...
post_command="${FSLDIR}/bin/bedpostx_postproc_gpu.sh --data=${subjdir}/data.nii --mask=$subjdir.bedpostX/nodif_brain_mask -b ${subjdir}.bedpostX/bvals -r ${subjdir}.bedpostX/bvecs  --forcedir --logdir=$subjdir.bedpostX/diff_parts $gopts $nvox $njobs ${subjdir} ${FSLDIR}"
# post_command="${FSLDIR}/bin/bedpostx_postproc_gpu.sh --data=${subjdir}/data.nii.gz --mask=$subjdir.bedpostX/nodif_brain_mask -b ${subjdir}.bedpostX/bvals -r ${subjdir}.bedpostX/bvecs  --forcedir --logdir=$subjdir.bedpostX/diff_parts $gopts $nvox $njobs ${subjdir} ${FSLDIR}"
