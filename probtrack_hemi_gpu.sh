...
if [ ! -f $output_file/fdt_matrix2.dot ] && [ ! -f $output_file/fdt_matrix2.npz ] ; then
    ...
    if [ $ANTS == False  ] ;  then
        #seeds=striatal.asc
        seeds="/home/test/lmq/Striatal_Subdivision/pipline/striatal_mask.nii"
        stop="/home/test/lmq/Striatal_Subdivision/pipline/cortex_mask.py"
        seedref=$t1/T1_brain.nii.gz
        echo 'fsl_mode'
        echo $seedref
        echo ${seeds}
        probtrackx2_gpu --samples=$bedpost_dir/merged \
        #--mask=$bedpost_dir/nodif_brain_mask \
        --mask=$t1/T1_brain_mask.nii.gz \
        #--xfm=$subdir/xfms/T1_1mm_2_DTI.mat \
        --seedref=$seedref \
        -P 5000 --loopcheck --forcedir -c 0.2 --sampvox=2 --randfib=1 \
        --stop=stop --forcefirststep  \
        -x seeds \
